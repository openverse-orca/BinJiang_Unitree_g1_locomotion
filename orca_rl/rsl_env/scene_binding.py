...
def _actuator_suffix_candidates(joint: str) -> list[str]:
    """执行器命名候选：joint 名、joint 名 + _mctrl、去 _joint 的旧命名。"""
    return [joint, f"{joint}_mctrl", joint.removesuffix("_joint")]
...
def resolve_g1_scene_binding(
    ...
    *,
    ...
    report = _scan_g1_scene_binding(orcagym_addr, time_step)
    ...
    actuator_suffixes = _resolve_g1_actuator_suffixes(agent_names[0], report.scene_names.actuators)
...
def _resolve_g1_actuator_suffixes(prefix: str, scene_actuators: set[str]) -> list[str]:
    """逐关节解析执行器后缀，容忍 _mctrl 等控制后缀的混合命名。"""
    resolved: list[str] = []
    missing: list[str] = []
    for joint in G1_JOINT_SUFFIXES:
        for candidate in _actuator_suffix_candidates(joint):
            if f"{prefix}_{candidate}" in scene_actuators:
    ...
    if missing:
        raise ValueError(
            f"G1 scene match is incomplete: actuators missing {missing} for agent {prefix!r}"
        )
    return resolved
